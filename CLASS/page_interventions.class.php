...
class page_interventions extends page_base {
	...
	public function les_interventions_mois()
	{
		if (isset ($_POST['MoisemployesI'])){$mois= $_POST['MoisemployesI'];}
	
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE MONTH(Date) = '$mois' ORDER BY Date ";
		$res = $this->connexion->query($req);
		return $res;
	}
	
	
	public function les_interventions_annee()
	{
		if (isset ($_POST['AnneeemployesI'])){$annee= $_POST['AnneeemployesI'];}
	
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE YEAR(Date) = '$annee' order by Date ";
		$res = $this->connexion->query($req);
		return $res;
	}
	
	public function les_interventions_moisannee()
	{
		if (isset ($_POST['AnneeemployesI'])){$annee= $_POST['AnneeemployesI'];}
		if (isset ($_POST['MoisemployesI'])){$mois= $_POST['MoisemployesI'];}
		
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE YEAR(Date) = '$annee' and MONTH(Date) = '$mois' order by Date ";
		$res = $this->connexion->query($req);
		return $res;
	}
	...
	public function les_interventions_employes2()
	{
	...
	public function les_interventions_employesMois()
	{
		...
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE EmplSage=$listeemployes and MONTH(Date) = '$mois' order by Date" ;
		$res = $this->connexion->query($req);
		return $res;
	}
	public function les_interventions_employesAnnee()
	{
		if (isset ($_POST['AnneeemployesI'])){$annee= $_POST['AnneeemployesI'];}
		if (isset($_POST['listeemployesI'])) {$listeemployes=$_POST['listeemployesI'];}
	
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE EmplSage=$listeemployes and YEAR(Date) = '$annee' order by Date" ;
		$res = $this->connexion->query($req);
		return $res;
	}
	public function les_interventions_employesMoisAnnee()
	{
		if (isset ($_POST['AnneeemployesI'])){$annee= $_POST['AnneeemployesI'];}
		if (isset($_POST['listeemployesI'])) {$listeemployes=$_POST['listeemployesI'];}
		if (isset ($_POST['MoisemployesI'])){$mois= $_POST['MoisemployesI'];}
	
		$req = "Select *, DAY(DATE) AS jour, MONTH(DATE) AS mois, YEAR(DATE) AS annee From interventions as I inner join employes as E on I.NUMEMPLSAGE=E.EmplSage inner join clients as C on I.CLIENTSAGE=C.CODESAGE WHERE EmplSage=$listeemployes and YEAR(Date) = '$annee' and MONTH(Date) = '$mois' order by Date" ;
		$res = $this->connexion->query($req);
		return $res;
	}
	...
	if (isset($_POST['listeemployesI'])){
		...
		else
		{
			...
			<form name='Excel' action=\"Excelinterv2.php\"  method=\"post\">
			    ...
			    <input type='hidden' name='MoisemployesI' id='MoisemployesI' value=".$mois.">
			    <input type='hidden' name='AnneeemployesI' id='AnneeemployesI' value=".$annee.">
			    <input type='hidden' name='listeemployes' id='listeemployes' value=".$listeemployes.">
					...
					<br>
				<label>!!! Cet export ne fonctionne pas !!!       </label><input name='Excel' type=\"submit\"style=\" width: 130px\"    value=\" Exporter \" />
			</form ></centre><br><nav >
